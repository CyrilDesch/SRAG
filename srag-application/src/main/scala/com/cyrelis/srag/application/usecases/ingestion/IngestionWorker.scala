...
import com.cyrelis.srag.application.usecases.ingestion.pipeline.preparator.PreparatorRouterPipeline
...
trait IngestionWorker {
  def run: ZIO[Any, Nothing, Unit]
}

object IngestionWorker {
  val layer: ZLayer[
    IngestionJobRepository[[X] =>> ZIO[Any, PipelineError, X]] & BlobStorePort & PreparatorRouterPipeline &
      IndexingPipeline & JobProcessingConfig & JobQueuePort,
    Nothing,
    IngestionWorker
  ] =
    ZLayer {
  ...
  private final class IngestionWorkerLive(
